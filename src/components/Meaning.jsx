...
const Meaning=({ meaning,setInput, setQueryParams})=> {

    const  removeUnwantedChars=(text)=> {
        // Remove single quotes, unwanted characters, and spaces
    ...
    const synonyms = meaning.synonyms.map((synonym, index) => <li key={index}   className="cursor-pointer hover:underline font-bold"
    onClick={() => {
        setInput(synonym);  // Update the input field with the synonym
        setQueryParams({ word: synonym });  // Update the query params with the synonym
     
      }} >{removeUnwantedChars(synonym)}</li>)
